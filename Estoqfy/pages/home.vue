...
export default {
  ...
  methods: {
    ...
    async getItems() {
      const response = await fetch(`https://localhost:7182/api/items?userId=${this.getCookie("userId")}`);
      if (response.ok) {
    ...
    async fetchDelete(id) {
      const response = await fetch(`https://localhost:7182/api/item?id=${id}`, { method: 'DELETE' });
      return response.ok;
    },
  },
};
